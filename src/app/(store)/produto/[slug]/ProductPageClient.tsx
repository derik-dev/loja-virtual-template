...
export default function ProductPage({ params }: ProductPageProps) {
  ...
  useEffect(() => {
    supabase.from('products').select('*').eq('slug', slug).single().then(({ data }) => {
      ...
      supabase.from('products').select('*').then(({ data: all }) => {
        const allMapped = (all ?? []).map(mapProduct)
        fetch('/api/ai/recommendations', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ currentProduct: p, allProducts: allMapped }),
        })
          .then((r) => r.json())
          .then((d) => setAiRecs(d.products ?? []))
          .catch(() => {})
        ...
        <div className="px-8 sm:px-14 lg:px-20">
          ...
          {aiRecsLoading ? (
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 sm:grid sm:grid-cols-4 sm:overflow-visible sm:pb-0">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="flex-shrink-0 w-48 sm:w-auto animate-pulse">
            ...
            <>
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 sm:hidden">
                {aiRecs.map((p) => (
                  <div key={p.id} className="flex-shrink-0 w-44">
                    <a href={`/produto/${p.slug}`} className="block group">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={p.images?.[0] ?? '/placeholder.png'}
                        alt={p.name}
                        className="w-full aspect-[3/4] object-cover bg-zinc-50 mb-2 group-hover:opacity-90 transition-opacity"
                      />
                      <p className="text-xs font-semibold text-zinc-900 leading-tight line-clamp-2 mb-1">{p.name}</p>
              ...
              <div className="hidden sm:grid grid-cols-4 gap-6">
                {aiRecs.map((p) => (
                  <a key={p.id} href={`/produto/${p.slug}`} className="block group">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={p.images?.[0] ?? '/placeholder.png'}
                      alt={p.name}
                      className="w-full aspect-[3/4] object-cover bg-zinc-50 mb-3 group-hover:opacity-90 transition-opacity"
                    />
                    <p className="text-sm font-semibold text-zinc-900 leading-tight line-clamp-2 mb-1">{p.name}</p>
                    <p className="text-sm text-zinc-500">
                      {p.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                    </p>
                  </a>
