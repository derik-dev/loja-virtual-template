...
export type Page = 'dashboard' | 'pedidos' | 'produtos' | 'categorias' | 'clientes' | 'descontos' | 'configuracoes'
    ...
    <aside className="w-56 flex-shrink-0 bg-zinc-900 flex flex-col min-h-screen">
      ...
      <nav className="flex-1 px-3 py-4 space-y-5">
        {NAV_SECTIONS.map((section, si) => (
          <div key={si}>
            {section.label && (
              <p className="px-3 mb-1 text-[9px] font-bold uppercase tracking-[0.18em] text-zinc-600">
                {section.label}
              </p>
            )}
      ...
      <div className="px-3 py-4 border-t border-zinc-800 space-y-0.5">
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 text-sm transition-colors rounded ${
            page === 'configuracoes' ? 'bg-white/10 text-white font-semibold' : 'text-zinc-400 hover:text-white hover:bg-white/5'
          }`}
        >
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
